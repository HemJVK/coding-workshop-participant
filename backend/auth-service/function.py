...
def handle_login(body):
    ...
    captcha_text = (body.get("captcha_text") or "").strip()
    captcha_hash = (body.get("captcha_hash") or "").strip()

    if not email or not password:
        return bad_request("Email and password are required")

    if not captcha_text or not captcha_hash:
        return bad_request("Captcha required")

    if generate_captcha_hash(captcha_text.upper()) != captcha_hash:
        return bad_request("Invalid captcha")
...
def handler(event=None, context=None):
    ...
    try:
        ...
        # Route: GET /auth-service/captcha
        if method == "GET" and path.endswith("/captcha"):
            return handle_get_captcha()
...
CAPTCHA_SECRET = os.getenv("CAPTCHA_SECRET", "captchasecretkey")

def generate_captcha_hash(text):
    return hashlib.sha256(f"{text}{CAPTCHA_SECRET}".encode('utf-8')).hexdigest()

def handle_get_captcha():
    text = ''.join(random.choices(string.ascii_uppercase + string.digits, k=5))
    image = ImageCaptcha(width=280, height=90)
    data = image.generate(text)
    base64_data = base64.b64encode(data.getvalue()).decode('utf-8')
    img_data_uri = f"data:image/png;base64,{base64_data}"

    hashed = generate_captcha_hash(text)
    return response(200, {"captcha_image": img_data_uri, "captcha_hash": hashed})
