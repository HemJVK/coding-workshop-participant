...
WORKSHOP_TOKEN = os.getenv("WORKSHOP_TOKEN", "workshop-9cf86d54-secret")


def should_require_workshop_token():
    # Only enforce the CloudFront-injected header in Lambda cloud runtime.
    return bool(os.getenv("AWS_LAMBDA_FUNCTION_NAME")) and os.getenv("IS_LOCAL", "false").lower() != "true"
...
async def unified_handler(service_name: str, request: Request, rest_of_path: str = ""):
    print(f"DEBUG: Internal request to service {service_name} with path {rest_of_path}")
    print(f"DEBUG: Headers: {dict(request.headers)}")

    # Verify request is from CloudFront when running behind the Lambda URL in cloud.
    if should_require_workshop_token() and request.headers.get("x-workshop-token") != WORKSHOP_TOKEN:
        if service_name != "health":
             return Response(status_code=403, content="Forbidden: Invalid Secret Header")

    if service_name == "health":
        return Response(content=json.dumps({"status": "healthy"}), status_code=200)
