...
def test_team_summary(analytics_mock_db):
    _, cur = analytics_mock_db
    token = auth_jwt.create_token(1, "admin@acme.com", "admin", "Admin User")

    cur.fetchone.return_value = (3, 2, 1, 2, 3)

    event = {
        "httpMethod": "GET",
        "path": "/analytics-service/team-summary",
        "headers": {"Authorization": f"Bearer {token}"},
    }
    ...
    assert body["leader_not_colocated_count"] == 2
    assert body["reporting_to_org_leader_count"] == 3
