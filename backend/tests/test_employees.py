...
def test_create_employee_hr_forbidden(employees_mock_db):
    token = auth_jwt.create_token(1, "hr@acme.com", "hr", "HR User")
    ...
    event = {
        ...
        "path": "/employees-service",
        "headers": {"Authorization": f"Bearer {token}"},
        "body": json.dumps({"name": "New Hire", "email": "new.hire@acme.com"})
    }
...
def test_create_employee_manager_forbidden(employees_mock_db):
